...
/* ISP ioctls from disassembly */
#define ISP_SENSOR_INFO 0x80786905  /* 120 bytes read — sensor vtable? */
#define ISP_CONFIG_07   0x40046907
#define ISP_CONFIG_08   0x40046908
#define ISP_CONFIG_09   0x40046909
#define ISP_INIT        0x690A
#define ISP_MODE_0F     0x4004690F
#define ISP_KICK        0x6910
#define ISP_START_20    0x40016920  /* 1 byte write */
#define ISP_CHIP_INFO   0x80086929
#define ISP_HW_CFG      0x80306926
...
int main(void){
    ...
    /* Try ALL ISP ioctls to find the one that starts processing */
    printf("[3] ISP ioctls:\n");fflush(stdout);

    ioctl(isp,ISP_CHIP_INFO,(uint32_t[]){0,0});
    ioctl(isp,ISP_HW_CFG,(uint32_t[12]){0});

    /* Try sensor info ioctl — 120 bytes, might register sensor */
    uint8_t sensor_buf[120];
    memset(sensor_buf, 0, 120);
    /* Fill with sensor info: name="cv2003_mipi", 1920x1080 */
    strcpy((char*)sensor_buf, "cv2003_mipi");
    *(uint16_t*)(sensor_buf+12) = 1920;
    *(uint16_t*)(sensor_buf+14) = 1080;
    *(uint16_t*)(sensor_buf+16) = 1920;
    *(uint16_t*)(sensor_buf+18) = 1080;
    ret=ioctl(isp,ISP_SENSOR_INFO,sensor_buf);
    printf("    SENSOR_INFO(0x80786905): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);

    /* ISP config ioctls */
    uint32_t val;
    val=0x203; /* Bayer format */
    ret=ioctl(isp,ISP_CONFIG_07,&val);
    printf("    CONFIG_07(fmt=0x203): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);

    val=1; /* enable */
    ret=ioctl(isp,ISP_CONFIG_08,&val);
    printf("    CONFIG_08(1): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);

    val=1;
    ret=ioctl(isp,ISP_CONFIG_09,&val);
    printf("    CONFIG_09(1): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);
    ...
    /* ISP mode */
    val=1;
    ret=ioctl(isp,ISP_MODE_0F,&val);
    printf("    MODE_0F(1): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);

    /* ISP start */
    uint8_t start=1;
    ret=ioctl(isp,ISP_START_20,&start);
    printf("    START_20(1): %d e=%d\n",ret,ret<0?errno:0);fflush(stdout);
    ...
    printf("---\n");fflush(stdout);
    system("cat /proc/driver/vpu 2>/dev/null | grep -E 'Width|Frame Rate|PIC_END|FINISH'");
    printf("---\n");fflush(stdout);
