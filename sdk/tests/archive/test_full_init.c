...
int main(void)
{
    ...
    L("[2] Devices: isp=%d vmm=%d\n",isp,vmm);
    if(isp<0||vmm<0){L("FATAL\n");return 1;}

    /* System memory */
    uint32_t sys_sz=0;
    int ret=ioctl(isp,VPSS_SYS_QUERY,&sys_sz);
    L("[3] SYS_QUERY: ret=%d size=%u\n",ret,sys_sz);

    if(ret==0 && sys_sz>0){
        uint32_t phys=0;
        ret=vmm_alloc(vmm,sys_sz,"vpu-sys",&phys);
        L("    VMM alloc: ret=%d phys=0x%08x\n",ret,phys);
        if(ret>=0){
            uint32_t m[3]={phys,0,sys_sz};
            ret=ioctl(isp,VPSS_SYS_SET_MEM,m);
            L("    SYS_SET_MEM: ret=0x%08x\n",(uint32_t)ret);
        }
    }

    /* Channel memory — query and allocate for each channel */
    L("[4] Channel memory init...\n");
    uint32_t ch_sizes[][2]={{1920,1080},{800,576},{512,288}};
    char *ch_names[]={"vpu-ch-0","vpu-ch-1","vpu-ch-2"};

    for(int ch=0;ch<3;ch++){
        uint32_t q[4]={ch,ch_sizes[ch][0],ch_sizes[ch][1],0};
        ret=ioctl(isp,VPSS_CHN_QUERY,q);
        uint32_t need=q[3];
        L("    CH%d query: ret=0x%08x need=%u\n",ch,(uint32_t)ret,need);

        if(ret==0 && need>0){
            uint32_t phys=0;
            ret=vmm_alloc(vmm,need,ch_names[ch],&phys);
            L("    CH%d alloc: ret=%d phys=0x%08x size=%u\n",ch,ret,phys,need);
            if(ret>=0){
                uint32_t s[6]={ch,0,0,0,phys,need};
                ret=ioctl(isp,VPSS_CHN_SET_MEM,s);
                L("    CH%d set: ret=0x%08x\n",ch,(uint32_t)ret);
            }
        }
    }

    /* Now try VPSS pipeline */
    L("\n[5] VPSS pipeline...\n");
    ...
    if(i2c>=0){
        ...
        L("    Regs %d/52, streaming ON\n",ok);
        close(i2c);
    }

    /* Get frame */
    L("\n[7] Get frame (waiting 500ms)...\n");
    usleep(500000);
