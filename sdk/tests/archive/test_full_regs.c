/* test_full_regs.c — Complete ISP register config from Ghidra.
 * Output to JFFS2 (survives reboot). No watchdog code. No TFTP checkpoints.
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(void){
    /* Write to JFFS2 so it survives reboot */
    FILE*out=fopen("/tmp/etc/config/result.txt","w");
    if(!out) out=fopen("/tmp/r.txt","w");
    if(!out) out=stdout;

    setvbuf(out,NULL,_IONBF,0); /* unbuffered — every write goes to disk */
    fprintf(out,"=== FULL REGS v3 ===\n");
    ...
    /* Feed watchdog so we don't get killed in debug mode */
    int wd=open("/dev/watchdog",O_WRONLY);
    if(wd<0)wd=open("/dev/watchdog0",O_WRONLY);
    if(wd<0)wd=open("/dev/watchdog",O_RDWR);
    if(wd>=0){write(wd,"\0",1);fprintf(out,"WD=%d\n",wd);}
    else{fprintf(out,"WD FAIL\n");}fflush(out);
    ...
    /* Will call 0x690A AFTER register writes + munmap */
    fprintf(out,"ISP ioctls OK\n");fflush(out);
    ...
    W(0x0B0,0x00E0D8D0);W(0x0B4,0);W(0x0B8,0);
    W(0x494,0x9015);
    ...
    W(0x02C,0x80);
    W(0x0F4,0x01010003);  /* processing mode control */
    W(0x030,0x00FEFF82);  /* vendor ISP config */
    /* NOTE: 0x0F0 stays in reset — will clear AFTER pkg_enable */
    fprintf(out,"REGS OK 030=%08x\n",RD(0x030));fflush(out);
    ...
    /* PkgEnable WHILE still in reset */
    W(0x100,4);W(0x034,5);R[0x100/4]=R[0x100/4]|1;
    /* Clear reset — ISP hardware ready to process */
    W(0x0F0,0x09);
    fprintf(out,"PKG+UNRESET: 100=%08x 0F0=%08x\n",RD(0x100),RD(0x0F0));

    /* CRITICAL: munmap ISP registers BEFORE isp_start to avoid
     * concurrent mmap conflict with kernel's ioremap */
    munmap((void*)R,0x4000); R=NULL;
    close(mem); mem=-1;
    fprintf(out,"MUNMAP OK\n");

    /* NOW register ISP interrupt handler — no mmap conflict */
    ioctl(isp,0x690A,(uint32_t[]){0});
    ioctl(isp,0x4004690F,(uint32_t[]){1});
    ioctl(isp,0x40016920,(uint8_t[]){1});
    ioctl(isp,0x6910,0);
    fprintf(out,"ISP_START+KICK OK\n");fflush(out);

    /* Sensor GPIO reset — toggle GPIO13 (capital G path on this camera) */
    system("echo 13 > /sys/class/gpio/export 2>/dev/null");
    system("echo out > /sys/class/GPIO13/direction 2>/dev/null");
    system("echo 0 > /sys/class/GPIO13/value 2>/dev/null;"
           "echo 0 > /sys/class/gpio/gpio13/value 2>/dev/null");
    usleep(50000);
    system("echo 1 > /sys/class/GPIO13/value 2>/dev/null;"
           "echo 1 > /sys/class/gpio/gpio13/value 2>/dev/null");
    usleep(50000);
    fprintf(out,"GPIO RST OK\n");

    /* Sensor — init + read back chip ID to verify I2C works */
    ...
    if(i2c>=0){
        ...
        /* Read back sensor chip ID (reg 0x3000-0x3001) to verify I2C */
        uint8_t rb[2]={0,0};
        uint8_t addr_buf[2]={0x30,0x00};
        struct i2c_msg rm[2]={
            {.addr=0x35,.flags=0,.len=2,.buf=addr_buf},
            {.addr=0x35,.flags=1,.len=2,.buf=rb}};
        struct i2c_rdwr_ioctl_data rx={.msgs=rm,.nmsgs=2};
        int rr=ioctl(i2c,I2C_RDWR,&rx);
        fprintf(out,"SNS ID=%02x%02x (r=%d)\n",rb[0],rb[1],rr);
        close(i2c);
    }
    ...
    usleep(500000);
    if(wd>=0)write(wd,"\0",1);
    /* Check interrupts — did ISP fire? */
    {FILE*p=fopen("/proc/interrupts","r");
    if(p){char l[256];while(fgets(l,sizeof(l),p))
        if(strstr(l,"ispp")||strstr(l,"mipi"))fprintf(out,"I:%s",l);
    fclose(p);}}
    ...
    if(wd>=0){write(wd,"V",1);close(wd);} /* magic close disables watchdog */

    /* Copy result to /tmp for TFTP */
