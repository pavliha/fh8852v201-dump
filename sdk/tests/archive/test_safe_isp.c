/* Safe ISP init: disable IRQ → write regs → enable IRQ */
#include <stdio.h>
#include <string.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(void){
    ...
    fprintf(out,"VPSS OK\n");fflush(out);
    /* ISP ioctls — DO NOT call ISP_INIT (0x690A) yet! */
    ...
    fprintf(out,"ISP IOCTLS pre-init\n");fflush(out);

    /* ISP hardware registers — write BEFORE isp_start (0x690A) */
    /* This way: no ISP interrupt handler running yet, safe to write */
    int mem=open("/dev/mem",2|0x80000);
    volatile uint32_t*R=0;
    if(mem>=0)R=mmap(0,0x3240,3,1,mem,0xE8400000);
    if(R&&R!=(void*)-1){
        /* Write all live register values EXCEPT 0x028 */
        ...
        fprintf(out,"ISP REGS written (before isp_start)\n");fflush(out);
    }

    /* NOW call ISP_INIT (0x690A) — this calls isp_start which registers
     * the ISP interrupt handler. The handler will see our pre-configured
     * registers and call init_isp on first interrupt. */
    ...
    /* Wait for ISP to process frames */
    sleep(3);

    /* Check results */
    if(R){
        fprintf(out,"REG 028=%08x 034=%08x 0F0=%08x 100=%08x\n",
                R[0x028/4],R[0x034/4],R[0x0F0/4],R[0x100/4]);fflush(out);
    }
