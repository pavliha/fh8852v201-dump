...
static int vmm_alloc(int vfd, uint32_t size, const char *name,
                     uint32_t *phys, uint32_t *virt)
{
    ...
    *(uint32_t*)(p+0x18)=0x103;
    r=ioctl(vfd,VMM_MMAP,p);
    *virt = (r>=0) ? *(uint32_t*)(p+0x14) : 0;
    return 0;
}
...
int main(void)
{
    ...
    printf("[2] Devices: isp=%d vmm=%d\n",isp,vmm);
    if(isp<0||vmm<0)return 1;

    /* SysInitMem */
    uint32_t sys_sz=0, phys, virt;
    int ret=ioctl(isp,VPSS_SYS_QUERY,&sys_sz);
    printf("[3] SysQuery: size=%u\n",sys_sz);
    if(ret==0 && sys_sz>0){
        vmm_alloc(vmm,sys_sz,"vpu-sys",&phys,&virt);
        printf("    Alloc: phys=0x%08x virt=0x%08x\n",phys,virt);
        uint32_t m[3]={phys,virt,sys_sz};
        ret=ioctl(isp,VPSS_SYS_SET_MEM,m);
        printf("    SetMem: 0x%08x %s\n",(uint32_t)ret,ret==0?"OK":"FAIL");
    }
    ...
    for(int ch=0;ch<3;ch++){
        ...
        if(ret==0 && need>0){
            ...
            char nm[16]; snprintf(nm,16,"vpu-ch-%d",ch);
            ret=vmm_alloc(vmm,need,nm,&ch_phys,&ch_virt);
            printf("phys=0x%08x virt=0x%08x ",ch_phys,ch_virt);
            if(ret==0 && ch_phys!=0){
                uint32_t s[6]={ch,ch_phys,ch_virt,need,res[ch][0],res[ch][1]};
                ret=ioctl(isp,VPSS_CHN_SET_MEM,s);
                printf("set=0x%08x %s\n",(uint32_t)ret,ret==0?"OK":"FAIL");
            } else printf("alloc fail\n");
        } else printf("query fail\n");
    }
