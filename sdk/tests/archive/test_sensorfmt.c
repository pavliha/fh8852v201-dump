/* test_sensorfmt.c — Add missing SetSensorFmt ioctl (0x40046906 = 0x203)
 * and check MIPI + IRQ status. Uses proven stable sequence.
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(void){
    ...
    /* NEW: SetSensorFmt — tell ISP the input format (0x203 for CV2003) */
    uint32_t fmt=0x203;
    int fret=ioctl(isp,0x40046906,&fmt);
    fprintf(out,"SENSORFMT=%d\n",fret);fflush(out);
    ...
    /* Mmap + register writes (proven stable) */
    ...
    usleep(1000);
    /* All ISP registers (same as before) */
    W(0x008,0x07FFFFFF);
    ...
    W(0x02C,0x80);
    /* Write vendor 030 — proven safe in 030_scan test */
    W(0x030,0x00FEFF82);
    ...
    system("busybox tftp -l /tmp/r.txt -r sensorfmt.txt -p 192.168.8.100 2>/dev/null");
