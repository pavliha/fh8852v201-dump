...
int main(void){
    ...
    /* MIPI init — BEFORE sensor starts! Configure 1-lane MIPI */
    printf("[2] MIPI\n"); fflush(stdout);
    uint8_t mipi[6]={0,1,0,0,0,0}; /* lane0=enable, others=0 */
    ret=ioctl(isp,MIPI_WRAP_INIT,mipi);
    printf("    wrap_init: %d\n",ret); fflush(stdout);
    ...
    uint32_t ip[4]={0};
    ioctl(isp,ISP_INIT,ip);
    printf("    init OK\n"); fflush(stdout);

    /* Sensor I2C init — AFTER MIPI is configured */
    printf("[5] Sensor\n"); fflush(stdout);
    ...
    /* ISP HW register init via /dev/mem */
    printf("[6] ISP regs\n"); fflush(stdout);
    int mem=open("/dev/mem",O_RDWR|O_SYNC);
    if(mem>=0){
        volatile uint32_t*R=mmap(NULL,0x3240,PROT_READ|PROT_WRITE,MAP_SHARED,mem,0xE8400000);
        if(R!=MAP_FAILED){
            /* Reset */
    ...
    /* Check MIPI */
    printf("[8] MIPI status:\n"); fflush(stdout);
    system("cat /proc/driver/mipi 2>/dev/null | grep -E 'Data rate|lane|width|heigh|sframe|status'");
