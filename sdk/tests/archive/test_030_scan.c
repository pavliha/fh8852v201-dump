/* test_030_scan.c — Find the right value for ISP config register 0x030.
 * Try incrementally enabling bits to start frame processing.
 * Writes output to JFFS2 (survives reboot).
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static void read_irq(FILE*out, const char*label){
    FILE*p=fopen("/proc/interrupts","r");
    if(!p)return;
    char line[256];
    while(fgets(line,sizeof(line),p)){
        if(strstr(line,"ispp")||strstr(line,"veu"))
            fprintf(out,"%s IRQ: %s",label,line);
    }
    fclose(p);fflush(out);
}
...
int main(void){
    ...
    /* System reset + ALL register writes (same as test_full_regs) */
    ...
    if(wd>=0)write(wd,"\0",1);
    usleep(500000); /* 0.5s for frames to arrive */
    if(wd>=0)write(wd,"\0",1);

    /* Check IRQ after sensor */
    read_irq(out,"POST");
    fprintf(out,"030=%08x 13C=%08x 140=%08x\n",RD(0x030),RD(0x13C),RD(0x140));fflush(out);

    /* Try GET_FRAME with current 030 */
    uint32_t fr[26];memset(fr,0,sizeof(fr));fr[0]=0;fr[22]=1000;
    int ret=ioctl(isp,0xC0686970,fr);
    fprintf(out,"F0=%08x\n",(uint32_t)ret);fflush(out);
    if(wd>=0)write(wd,"\0",1);

    /* Now try writing 030 to vendor value and check if frame counters change */
    fprintf(out,"TRYING 030 values:\n");fflush(out);

    /* Try incrementally: kernel default with just a few bits changed */
    static const uint32_t try_vals[] = {
        0x0096DF82,  /* just clear bit 0 */
        0x0096FF83,  /* enable bit 13 (byte 1) */
        0x00D6DF83,  /* enable bit 22 */
        0x00F6DF83,  /* enable bits 21,22 */
        0x0096FF82,  /* clear bit 0 + enable bit 13 */
        0x00FEFF82,  /* full vendor value */
    };
    ...
    for(int t=0;t<6;t++){
        ...
        W(0x030,try_vals[t]);
        usleep(200000);
        uint32_t actual=RD(0x030);
        uint32_t cnt=RD(0x13C);
        fprintf(out,"  030=%08x→%08x 13C=%08x",try_vals[t],actual,cnt);

        memset(fr,0,sizeof(fr));fr[0]=0;fr[22]=500;
        ret=ioctl(isp,0xC0686970,fr);
        fprintf(out," F=%08x\n",(uint32_t)ret);fflush(out);

        if(ret==0){
            fprintf(out,"*** GOT FRAME at 030=%08x! ***\n",try_vals[t]);
